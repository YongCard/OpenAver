...
def _source_enum_with_stash(include_auto: bool = False) -> list[str]:
    values = get_source_enum(include_auto=include_auto)
    return values if "stash" in values else [*values, "stash"]
...
_TOOLS: list[dict] = [
    {
        ...
        "input_schema": {
            ...
            "properties": {
                ...
                "source": {
                    "type": "string",
                    "enum": _source_enum_with_stash(include_auto=False),
                    "description": "指定來源（可選；stash 用於本機 Stash 歐美影片）",
                },
                "since": {
                    "type": "string",
                    "format": "date",
    ...
    {
        "name": "parse_filename",
        "description": "從檔名提取番號、字幕偵測，並識別欧美 studio.date.performer.title 場景。不修改資料庫",
        "side_effect": False,
        "method": "POST",
        ...
        "output_schema": {
            "results": "[{filename, number, has_subtitle, media_type, scene_query, studio, date, performer, title}] — 解析結果陣列",
        },
        "retry_safe": True,
    ...
    {
        "name": "stash_connection_test",
        "description": "測試本機 Stash GraphQL 連線，用於欧美影片刮削來源。不修改資料庫",
        "side_effect": False,
        "method": "POST",
        "path": "/api/settings/stash/test",
        "input_schema": {
            "type": "object",
            "properties": {
                "url": {"type": "string", "description": "Stash server URL，例如 http://127.0.0.1:9999"},
                "api_key": {"type": "string", "description": "Stash ApiKey，可為空"},
            },
            "required": ["url"],
        },
        "output_schema": {
            "success": "boolean",
            "version": "string — Stash 版本（可為空）",
            "message": "string — 可讀連線結果",
        },
        "retry_safe": True,
        "_example_template": "curl -X POST -H 'Content-Type: application/json' -d '{{\"url\":\"http://127.0.0.1:9999\",\"api_key\":\"\"}}' {base}/api/settings/stash/test",
    },
    {
        "name": "enrich_single",
        ...
        "input_schema": {
            ...
            "properties": {
                ...
                "source": {
                    "type": "string",
                    "enum": _source_enum_with_stash(include_auto=True),
                    "default": "auto",
...
_TOOLS.extend([
    {
        ...
        "description": "盤點既有影片庫並備份 NFO、圖片、字幕、設定與資料庫；不搬移影片，預設跳過 #待整理 / #待人工整理 / 未整理",
        "method": "POST",
        ...
        "input_schema": {
            ...
            "properties": {
                ...
                "include_manual": {
                    "type": "boolean",
                    "default": False,
                    "description": "是否納入 #待整理 / #待人工整理 / 未整理 內影片；僅重新識別人工項目時設為 true",
                },
            },
            "required": ["root"],
        },
    ...
    },
    {
        ...
        "input_schema": {
            ...
            "properties": {
                ...
                "manual_folder": {"type": "string", "default": "#待整理"},
            },
            "required": ["run_dir"],
        },
        "output_schema": {
    ...
    {
        "name": "library_duplicate_numbers",
        "description": "檢測本地資料庫中同番號多影片；合法 CD1/CD2、Part1/Part2 多段預設隱藏。不修改資料庫或檔案",
        "method": "GET",
        "path": "/api/library-migration/duplicates",
        "input_schema": {
            "type": "object",
            "properties": {
                "include_multipart": {"type": "boolean", "default": False},
                "include_missing_paths": {"type": "boolean", "default": True},
                "limit": {"type": "integer", "minimum": 1, "maximum": 5000, "default": 500},
            },
        },
        "output_schema": {
            "summary": "{total_numbers, duplicate_group_count, multipart_group_count, duplicate_file_count, missing_path_count}",
            "groups": "[{number, classification, reason, count, variant_tags, part_labels, items}]",
        },
        "side_effect": False,
        "confirmation_required": False,
        "idempotent": True,
        "retry_safe": True,
        "_example_template": "curl '{base}/api/library-migration/duplicates?include_multipart=false&limit=500'",
    },
    {
        "name": "library_duplicate_delete_preview",
        "description": "預覽刪除單個疑似重複影片時會送入回收站的影片與同 stem 伴隨檔案；不修改資料庫或檔案",
        "method": "POST",
        "path": "/api/library-migration/duplicate-delete/preview",
        "input_schema": {
            "type": "object",
        ...
        "output_schema": {
            ...
            "file_count": "integer",
            "total_bytes": "integer",
            "files": "[{path, name, size, kind}]",
        },
        "side_effect": False,
        "confirmation_required": False,
        "idempotent": True,
        "retry_safe": True,
        "_example_template": "curl -X POST -H 'Content-Type: application/json' -d '{{\"path\":\"D:/library/ABC-123.mp4\"}}' {base}/api/library-migration/duplicate-delete/preview",
    },
    {
        "name": "library_duplicate_delete_apply",
        "description": "將單個疑似重複影片及同 stem 伴隨檔案送入 Windows 回收站，成功後刪除對應資料庫記錄；不永久刪除",
        "method": "POST",
        "path": "/api/library-migration/duplicate-delete/apply",
        "input_schema": {
            "type": "object",
            "properties": {
                "path": {"type": "string"},
        ...
        "output_schema": {
            "success": "boolean",
            "deleted_db_rows": "integer",
            "moved_to_recycle_bin": "integer",
            "files": "[{path, name, size, kind}]",
        },
        "side_effect": True,
        "confirmation_required": True,
        "idempotent": False,
        "retry_safe": False,
        "_example_template": "curl -X POST -H 'Content-Type: application/json' -d '{{\"path\":\"D:/library/ABC-123.mp4\",\"confirm\":true}}' {base}/api/library-migration/duplicate-delete/apply",
    },
    {
        "name": "library_empty_folders_preview",
        "description": "檢測設定資料庫目錄中的空資料夾；跳過 #待整理、#待人工整理、.openaver-migration、未整理，不修改檔案",
        "method": "POST",
        "path": "/api/library-migration/empty-folders/preview",
        "input_schema": {
            "type": "object",
        ...
        "output_schema": {
            ...
            "folders": "[{path, root, name}]",
            "skipped_protected_count": "integer",
        ...
        "idempotent": True,
        "retry_safe": True,
        "_example_template": "curl -X POST -H 'Content-Type: application/json' -d '{{}}' {base}/api/library-migration/empty-folders/preview",
    },
    {
        "name": "library_empty_folders_apply",
        "description": "將設定資料庫目錄中的空資料夾送入 Windows 回收站；不永久刪除，必須確認",
        "method": "POST",
        "path": "/api/library-migration/empty-folders/apply",
        "input_schema": {
            "type": "object",
            "properties": {
        ...
        "output_schema": {
            "success": "boolean",
            "removed_empty_folder_count": "integer",
            "removed_empty_folders": "[{path, root, name}]",
            "warnings": "array",
        },
        "side_effect": True,
        "confirmation_required": True,
        "idempotent": False,
        "retry_safe": False,
        "_example_template": "curl -X POST -H 'Content-Type: application/json' -d '{{\"confirm\":true}}' {base}/api/library-migration/empty-folders/apply",
    },
    {
        "name": "library_title_placeholder_preview",
        "description": "預覽將標題未定、未知標題、空標題或標題等於番號的影片隔離到 #待整理；不刮削、不翻譯、不修改檔案",
        "method": "POST",
        "path": "/api/library-migration/title-placeholder/preview",
        "input_schema": {
            "type": "object",
        ...
        "output_schema": {
            "run_id": "string",
            "manifest": "string",
            "summary": "{candidate_count, planned_count, conflict_count, sidecar_count}",
            "entries": "[{number, title, reason, source, target, sidecars, status}]",
        },
        "side_effect": False,
        "confirmation_required": False,
        "idempotent": True,
        "retry_safe": True,
        "_example_template": "curl -X POST -H 'Content-Type: application/json' -d '{{}}' {base}/api/library-migration/title-placeholder/preview",
    },
    {
        "name": "library_title_placeholder_apply",
        "description": "按固定清單將標題不可信影片與同 stem 伴隨檔移到 #待整理；更新資料庫路徑；每批最多 20 部",
        "method": "POST",
        "path": "/api/library-migration/title-placeholder/apply",
        "input_schema": {
            "type": "object",
            "properties": {
                "manifest": {"type": "string"},
        ...
        "output_schema": {
            "success": "boolean",
            "moved_entries": "integer",
            "updated_db_rows": "integer",
            "remaining": "integer",
            "skipped": "array",
            "removed_empty_folder_count": "integer",
        },
        ...
        "idempotent": False,
        "retry_safe": False,
        "_example_template": "curl -X POST -H 'Content-Type: application/json' -d '{{\"manifest\":\"/library/.openaver-migration/demo/title_placeholder_manifest.json\",\"confirm\":true,\"batch_size\":20}}' {base}/api/library-migration/title-placeholder/apply",
    },
    {
        "name": "library_title_placeholder_rollback",
        "description": "按標題未定隔離 journal 逆序恢復已移動的影片與伴隨檔；必須確認",
        "method": "POST",
        "path": "/api/library-migration/title-placeholder/rollback",
        "input_schema": {
            "type": "object",
            "properties": {
                "manifest": {"type": "string"},
        ...
        "idempotent": False,
        "retry_safe": False,
        "_example_template": "curl -X POST -H 'Content-Type: application/json' -d '{{\"manifest\":\"/library/.openaver-migration/demo/title_placeholder_manifest.json\",\"confirm\":true,\"batch_size\":20}}' {base}/api/library-migration/title-placeholder/rollback",
    },
    {
        "name": "inbox_organizer_roots",
        "description": "列出目前資料庫根目錄及各自 #待整理 投遞箱狀態；不修改檔案",
        "method": "GET",
        "path": "/api/inbox-organizer/roots",
        "input_schema": {"type": "object", "properties": {}, "required": []},
        "output_schema": {
            "manual_folder": "string — 固定為 #待整理",
            "roots": "[{root, inbox, exists, video_count}]",
        },
        "side_effect": False,
        "confirmation_required": False,
        "idempotent": True,
        "retry_safe": True,
        "_example_template": "curl '{base}/api/inbox-organizer/roots'",
    },
    {
        "name": "inbox_organizer_inventory",
        "description": "掃描資料庫根目錄下的 #待整理，只列出影片；無法識別番號也保留在結果中",
        "method": "POST",
        "path": "/api/inbox-organizer/inventory",
        "input_schema": {
            "type": "object",
            "properties": {"root": {"type": "string", "description": "可選；限制只掃描單一資料庫根目錄"}},
        },
        "output_schema": {
            "summary": "{file_count, identified_count, needs_number_count}",
            "entries": "[{id, root, source, filename, number, status, reason, size, mtime}]",
        },
        "side_effect": False,
        "confirmation_required": False,
        "idempotent": True,
        "retry_safe": True,
        "_example_template": "curl -X POST -H 'Content-Type: application/json' -d '{{}}' {base}/api/inbox-organizer/inventory",
    },
    {
        "name": "inbox_organizer_search",
        "description": "對 #待整理 inventory 條目自動搜尋 metadata；無法識別或搜不到的條目保留供人工輸入番號/指定來源重搜",
        "method": "POST",
        "path": "/api/inbox-organizer/search",
        "input_schema": {
            "type": "object",
            "properties": {
                "entries": {"type": "array", "items": {"type": "object"}},
                "source": {"type": "string", "default": "auto", "description": "auto 或指定來源如 dmm/javbus/javdb"},
            },
            "required": ["entries"],
        },
        "output_schema": {
            "summary": "{total, found_count, needs_review_count}",
            "entries": "[inventory entries with metadata/status/reason]",
        },
        "side_effect": False,
        "confirmation_required": False,
        "idempotent": True,
        "retry_safe": True,
        "_example_template": "curl -X POST -H 'Content-Type: application/json' -d '{{\"entries\":[],\"source\":\"auto\"}}' {base}/api/inbox-organizer/search",
    },
    {
        "name": "inbox_organizer_search_job_start",
        "description": "啟動 #待整理 背景搜尋任務；切換頁面不中斷，但不跨 OpenAver 重啟保留。selected_ids 可只重搜選中項",
        "method": "POST",
        "path": "/api/inbox-organizer/search-jobs",
        "input_schema": {
            "type": "object",
            "properties": {
                "entries": {"type": "array", "items": {"type": "object"}},
        ...
        "output_schema": {
            "job": "{job_id, phase, current, total, current_label, summary, logs, entries}",
        },
        "side_effect": True,
        "confirmation_required": False,
        "idempotent": False,
        "retry_safe": False,
        "_example_template": "curl -X POST -H 'Content-Type: application/json' -d '{{\"entries\":[],\"source\":\"auto\"}}' {base}/api/inbox-organizer/search-jobs",
    },
    {
        "name": "inbox_organizer_search_job_current",
        "description": "取得目前或最近完成的 #待整理 背景搜尋任務；任務只保留於目前 OpenAver 進程記憶體",
        "method": "GET",
        "path": "/api/inbox-organizer/search-jobs/current",
        "input_schema": {"type": "object", "properties": {}, "required": []},
        "output_schema": {
            "job": "null 或 {job_id, phase, current, total, current_label, summary, logs, entries}",
        },
        "side_effect": False,
        "confirmation_required": False,
        "idempotent": True,
        "retry_safe": True,
        "_example_template": "curl '{base}/api/inbox-organizer/search-jobs/current'",
    },
    {
        "name": "inbox_organizer_search_job_status",
        "description": "依 job_id 查詢 #待整理 背景搜尋進度、最近日志和當前 entries",
        "method": "GET",
        "path": "/api/inbox-organizer/search-jobs/{job_id}",
        "input_schema": {
            "type": "object",
        ...
        "idempotent": True,
        "retry_safe": True,
        "_example_template": "curl '{base}/api/inbox-organizer/search-jobs/<job_id>'",
    },
    {
        "name": "inbox_organizer_search_job_cancel",
        "description": "請求取消 #待整理 背景搜尋；已完成條目保留，未處理條目標記為 canceled",
        "method": "POST",
        "path": "/api/inbox-organizer/search-jobs/{job_id}/cancel",
        "input_schema": {
            "type": "object",
            "properties": {"job_id": {"type": "string"}},
            "required": ["job_id"],
        ...
        "output_schema": {
            "job": "{job_id, phase, current, total, summary, logs, entries}",
        },
        "side_effect": True,
        "confirmation_required": False,
        "idempotent": True,
        "retry_safe": True,
        "_example_template": "curl -X POST '{base}/api/inbox-organizer/search-jobs/<job_id>/cancel'",
    },
    {
        "name": "inbox_organizer_plan",
        "description": "根據固定 entries 生成整理預覽；目標根目錄永遠是資料庫根目錄，不是 #待整理；不搬移檔案",
        "method": "POST",
        "path": "/api/inbox-organizer/plan",
        "input_schema": {
            "type": "object",
            "properties": {
                "entries": {"type": "array", "items": {"type": "object"}},
        ...
        "output_schema": {
            "manifest": "string — 固定整理清單",
            "summary": "{total, planned_count, conflict_count, skipped_count}",
            "entries": "[{source, target, number, title, sidecars, status, reason}]",
        },
        "side_effect": True,
        "confirmation_required": False,
        "idempotent": True,
        "retry_safe": True,
        "_example_template": "curl -X POST -H 'Content-Type: application/json' -d '{{\"entries\":[]}}' {base}/api/inbox-organizer/plan",
    },
    {
        "name": "inbox_organizer_apply",
        "description": "套用固定 #待整理 manifest；一次整理所有可搬移項，不重新搜尋、不覆蓋已有檔案，失敗項留在 #待整理",
        "method": "POST",
        "path": "/api/inbox-organizer/apply",
        "input_schema": {
            "type": "object",
            "properties": {
                "manifest": {"type": "string"},
                "confirm": {"type": "boolean", "const": True},
                "batch_size": {"type": "integer", "minimum": 1, "maximum": 10000, "default": 9999},
            },
            "required": ["manifest", "confirm"],
        },
        "output_schema": {
        ...
        "idempotent": True,
        "retry_safe": True,
        "_example_template": "curl -X POST -H 'Content-Type: application/json' -d '{{\"manifest\":\"/library/.openaver-migration/demo/inbox_organizer_manifest.json\",\"confirm\":true,\"batch_size\":10000}}' {base}/api/inbox-organizer/apply",
    },
    {
        "name": "inbox_organizer_rollback",
        "description": "按 #待整理 journal 逆序回滾最近批次，恢復已移動影片與伴隨檔；必須確認",
        "method": "POST",
        "path": "/api/inbox-organizer/rollback",
        "input_schema": {
            "type": "object",
            "properties": {
                "manifest": {"type": "string"},
        ...
        "idempotent": False,
        "retry_safe": False,
        "_example_template": "curl -X POST -H 'Content-Type: application/json' -d '{{\"manifest\":\"/library/.openaver-migration/demo/inbox_organizer_manifest.json\",\"confirm\":true,\"batch_size\":10000}}' {base}/api/inbox-organizer/rollback",
    },
])
