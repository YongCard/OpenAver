...
驗證 web/app.py get_lan_ip() helper：
  - 正常路徑：回傳非空字串（區網 IP）
...
def test_get_lan_ip_returns_string_or_none():
    ...
    本測試允許兩種情況，確保不拋例外。
    """
    from web.app import get_lan_ip
    result = get_lan_ip()
...
def test_get_lan_ip_no_exception_propagated(monkeypatch):
    ...
    monkeypatch.setattr(socket, "socket", _BustedSocket)
...
def test_get_lan_ip_closes_socket_on_error(monkeypatch):
    ...
    monkeypatch.setattr(socket, "socket", _TrackSocket)
